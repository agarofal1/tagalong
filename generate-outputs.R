...
outdir <- args[8]
script.dir <- this.dir() 
refdir <- "/drive3/staging/agarofal/tagalong/reference/"

virus.dict <- fread(file.path(refdir, "viral_id_map_table.txt"), header=F, sep="\t", stringsAsFactors = F) %>% dplyr::rename(id=1, name=2)
...
extract.breaks <- function(clust){
  ...
  cdhit.command <- paste("sh", file.path(script.dir, "run-cdhit.sh"), sample, human.fa.path, outdir, clust.perc, sep=" ")
  system(cdhit.command)
  cdhit.command2 <- paste("sh", file.path(script.dir, "run-cdhit.sh"), sample, viral.fa.path, outdir, clust.perc, sep=" ")
  ...
  output.vec <- c(sample, clust, human.breakchr, human.breakpos, human.type, human.gene, human.seq, virus.id, virus.name, virus.breakpos, viral.gene, viral.seq, strm, junc.seq, read.support)
  return(output.vec)
}

if(nrow(blast.frame) != 0){
  output.table <- do.call(rbind.data.frame, unname(lapply(unique(blast.frame$cluster), function(x) extract.breaks(x)))) %>% dplyr::rename(sample=1, cluster_name=2, human_chr=3, human_breakpos=4, human_region=5, human_gene=6, human_seq=7, virus_id=8, virus_name=9, virus_breakpos=10, virus_gene=11, virus_seq=12, break_ori=13, breakpt_seq=14, frag_support=15)
  
  write.table(output.table, file=paste(outdir, "/", sample, ".integration.calls.txt", sep=""), col.names=T, row.names=F, sep="\t", quote=F)
} else{
  print("No reads remaining after BLAST filtering. Exiting.")
  empty.command <- paste("echo \"No viral integration sites detected.\" >", paste(outdir, "/", sample, ".integration.calls.txt", sep=""))
  system(empty.command)
}
