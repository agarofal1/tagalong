...
if [[ ! -e "$outdir"/${sample}.softclip.q"$mapq_thresh".reads.sam ]]; then 
	...
	samtools view -F 2048 -q "$mapq_thresh" -L $sigbed $bam | awk '$6 ~ "S"' > $outdir/${sample}.softclip.q"$mapq_thresh".reads.sam
fi
...
Rscript $scriptDir/process-softclips.R $sample $outdir/${sample}.leftclip.q"$mapq_thresh".reads.txt $outdir/${sample}.rightclip.q"$mapq_thresh".reads.txt $clip_length $mapq_thresh $clust_support $outdir 
